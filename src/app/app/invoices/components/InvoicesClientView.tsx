...
export function InvoicesClientView() {
  // View & Filter State
    ...
    return invoices.filter(inv => {
      ...
      if (debouncedSearch) {
        ...
        return (
          ...
          (inv.client?.contact_name || '').toLowerCase().includes(s) ||
  ...
  const totalValue = filteredInvoices?.reduce((acc, i) => acc + i.total, 0) || 0;
  const paidValue = filteredInvoices?.filter((i) => i.status === 'paid').reduce((acc, i) => acc + i.total, 0) || 0;
  const overdueValue = filteredInvoices?.filter((i) => i.status === 'overdue').reduce((acc, i) => acc + i.total, 0) || 0;
    ...
    <>
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        ...
        <Button onClick={() => setCreateInvoiceOpen(true)}>
          <Plus className="mr-2 h-4 w-4" /> Nová faktúra
        </Button>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                {isLoading ? (
                  ...
                  <p className="text-2xl font-semibold">{filteredInvoices?.length || 0}</p>
                )}
              </div>
                ...
                <TableRow>
                  <TableCell colSpan={7} className="p-0">
                    <EmptyState
                      ...
                      title={searchQuery ? "Nenašli sa žiadne výsledky" : "Zatiaľ žiadne faktúry"}
                      description={
                        searchQuery
                          ? `Pre výraz "${searchQuery}" sme nenašli žiadnu zhodu.`
                          : "Vytvorte faktúru z akceptovanej ponuky alebo od začiatku."
                      }
                      action={{
